    ...
    for (var i = 0; i < tr.length; i++) {
        ...
        if (td) {
            ...
            if (rowTxt.toUpperCase().indexOf(filter) > -1) {
                ...
                tr[i].style.display = "none"; // If not, don't display
            }
        }
    }
}
...
function validateEditForm() {
    ...
    var wins = document.forms["login"]["wins"].value;
    var losses = document.forms["login"]["losses"].value;
    var ties = document.forms["login"]["ties"].value;
    var admin = document.forms["login"]["admin"].value;

    if (username == "" || password == "" || fname == "" || lname == "" || wins == "" || losses == "" || ties == "" || admin == "") {
